...
// src/utils/flightSearchLogic.ts
function normalizeFlightPrice(value) {
...
function getPricedSerpApiFlights(rawFlights = []) {
  return rawFlights.reduce((acc, flight) => {
  ...
  try {
    ...
    if (serpApiKey) {
      try {
        ...
        console.log(`
\u{1F4E1} [SERPAPI ENGINE] Querying SerpAPI Google Flights...`);
        console.log(`[SERPAPI_REQUEST] route=${origin}-${destination} departure=${departDate || "N/A"} return=${returnDate || "N/A"}`);
        console.log(`\u{1F4CD} Parameters: route=${origin}-${destination}, dep=${departDate}, ret=${returnDate}, class=${cabinClass}`);
        ...
        if (serpRes.ok) {
          ...
          if (pricedFlights.length > 0) {
            ...
            realTimeFlights = pricedFlights.slice(0, 8).map(({ flight, price }, idx) => {
...
if (!process.env.VERCEL && process.env.VERCEL_ENV === void 0) {
  startServer();
}
